...
Time TimeCreate(int hh, int mm, int ss) {
  ...
  Time t = (hh*3600) + (mm*60) + ss;  // Fill in the formula for total seconds since midnight
...
int TimeCompare(Time a, Time b) {
   if(TimeGetSeconds(a) > TimeGetSeconds(b)){
     return 1;
   }else if(TimeGetSeconds(a) < TimeGetSeconds(b)){
     return -1;
   }else{
     return 0;
   }
